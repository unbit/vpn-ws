	...
	if (vpn_ws_conf.gid) {
		...
		if (!g) {
			if (vpn_ws_is_a_number(vpn_ws_conf.gid)) {
				...
				vpn_ws_warning("unable to find group %s", vpn_ws_conf.gid);
				vpn_ws_exit(1);
			}
		}
		else {
		...
		if (!gid) {
			vpn_ws_warning("unable to drop to gid");
			vpn_ws_exit(1);
		}
		if (setgid(gid)) {
			vpn_ws_error("setgid()");
	...
	if (vpn_ws_conf.uid) {
                ...
                if (!p) {
                        if (vpn_ws_is_a_number(vpn_ws_conf.uid)) {
                                ...
                                vpn_ws_warning("unable to find user %s", vpn_ws_conf.uid);
                                vpn_ws_exit(1);
                        }
                }
                else {
                ...
                if (!uid) {
                        vpn_ws_warning("unable to drop to uid");
                        vpn_ws_exit(1);
                }
                if (setuid(uid)) {
                        vpn_ws_error("setuid()");
